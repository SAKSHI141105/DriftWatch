...
export function ReasonExplainer({ reason, attackType, confidence }: ReasonExplainerProps) {
    ...
    return parts.map((part, i) => {
      if (
        ...
        return (
          <span
            key={i}
            className="font-mono text-amber-500 bg-amber-500/10 px-1.5 rounded border border-amber-500/20 font-medium"
          >
            {part}
          </span>
        );
  ...
  return (
    <div className="rounded-xl bg-card border border-amber-500/30 p-5 shadow-[0_4px_20px_rgba(0,0,0,0.4)] relative overflow-hidden">
      {/* Left accent stripe */}
      ...
      <div className="flex items-start gap-3.5">
        ...
        <div className="space-y-2 flex-1 min-w-0">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <h4 className="text-xs font-mono tracking-wider uppercase text-muted-foreground flex items-center gap-1.5">
            ...
            <div className="flex items-center gap-2 font-mono text-xs">
              <span className="text-muted-foreground uppercase">{formattedType}</span>
              {confidencePct && (
